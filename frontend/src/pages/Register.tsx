    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 py-8 text-white">
      <section className="w-full max-w-md space-y-8 rounded-3xl bg-slate-900/70 p-10 shadow-glow">
        <header className="space-y-3 text-center">
          ...
          <p className="text-sm text-slate-300">Start tracking your readiness insights.</p>
        </header>
        ...
        <form className="space-y-5" onSubmit={onSubmit}>
          <label className="block text-sm">
            ...
            <input
              type="text"
              className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-900/70 px-4 py-3 focus:border-brand-aqua focus:outline-none"
              placeholder="Avery Lee"
              {...register('name', { required: 'Name is required' })}
            />
            {errors.name && <span className="mt-1 block text-xs text-rose-400">{errors.name.message}</span>}
          </label>
          ...
          <label className="block text-sm">
            ...
            />
            {errors.password && (
              <span className="mt-1 block text-xs text-rose-400">Password must be at least 6 characters</span>
            )}
          </label>
          ...
          <button
            ...
            className="w-full rounded-2xl bg-brand-aqua/80 py-3 text-sm font-semibold text-white transition hover:bg-brand-aqua disabled:opacity-70"
          >
        ...
        <p className="text-center text-sm text-slate-300">
          ...
          <Link to="/login" className="text-brand-neon hover:text-brand-aqua">
            Sign in
          </Link>
        </p>
      </section>
